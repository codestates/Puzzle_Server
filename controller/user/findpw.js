...
module.exports = async (req, res) => {
    const { name, email, changePw, phone } = req.body
    ...
    if (!changePw) {
        ...
        if (!findUser) {
            ...
            res.status(200).json({ "message": "ok", "data": { eamil: email, name: name, phone: phone } })
        }
        ...
        const editedUser = await user.update({
            password: sha256(changePw + process.env.SALT)
